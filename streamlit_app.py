...
def make_response_df(response_df: pd.DataFrame) -> pd.DataFrame:
  ...
  response_df["width"] = response_df[[0, 1]].apply(lambda x: np.sqrt(np.sum(np.square(np.array(x[0]) - np.array(x[1])))), axis=1)
  response_df["height"] = response_df[[0, 3]].apply(lambda x: np.sqrt(np.sum(np.square(np.array(x[0]) - np.array(x[3])))), axis=1)
...
def get_cluster(input_df: pd.DataFrame, n_books:int) -> pd.DataFrame:
  X = np.array([[i, j] for i, j in zip(input_df["a"], input_df["b"])])
...
def make_book_df(title_list:List[str], n_books:int) -> pd.DataFrame:
    ...
    book_df = pd.concat(book_df_list)[
        ["Item.title", "Item.author", "Item.publisherName", "Item.salesDate", "Item.isbn", "Item.itemCaption"]].rename(
        columns=
        {"Item.title": "書名",
          "Item.author": "著者",
...
def main():
    ...
    if st.session_state.initial_load:
        ...
        if st.session_state.n_books > 0:
                ...
                book_list = response_df.groupby("cluster").aggregate({"area": lambda x: np.argmax(x), "text": lambda x: x.tolist()}).apply(
